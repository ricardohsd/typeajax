(function ($, _) {
  ...
  Typeajax.prototype = {
    ...
    fetch: function (query, process) {
      ...
      process([JSON.stringify({value: '<i class="icon-spinner icon-spin"></i> Searching'})]);
      ...
      this.jqXHR.done(function (data) {
        ...
        if (data.length) {
          ...
          process([JSON.stringify({value: '<i class="icon-thumbs-down"></i> We\'re sorry, but nothing was found for the term "' + query + '"'})]);
        }
      });
      ...
      this.jqXHR.fail(function (jqXHR, textStatus, errorThrown) {
        ...
        process([JSON.stringify({value: '<i class="icon-warning-sign"></i> We\'re sorry, but something went wrong (' + jqXHR.status + ')'})]);
      });
    },
